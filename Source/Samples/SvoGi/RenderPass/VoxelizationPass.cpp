...
void VoxelizationPass::do_rebuild_pixel_data_buffers() {
    ...
    kVoxelizationMeta.TotalLevel = (uint32_t)std::ceil(std::log2f((float)mVoxelGridResolution_));   // last level in brick pool
...
void VoxelizationPass::do_rebuild_svo_buffers() {
    ...
    mpLevelAddressBuffer_ = Buffer::create((kVoxelizationMeta.TotalLevel + 1u) * sizeof(uint32_t));
}
...
void VoxelizationPass::do_clear(RenderContext* pContext) {
    ...
    levleAddressBufInit.resize(kVoxelizationMeta.TotalLevel + 1u, std::numeric_limits<uint32_t>::max());
    levleAddressBufInit[kVoxelizationMeta.TotalLevel] = 0;
    mpLevelAddressBuffer_->setBlob(levleAddressBufInit.data(), 0, sizeof(uint32_t) * (kVoxelizationMeta.TotalLevel + 1u));
}
